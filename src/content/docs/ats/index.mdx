...
| **End-user identity**     | Abstracted; user holds the access code, **or** ref-scoped via your own `external_user_ref` | Abstracted; you correlate via opaque `external_user_ref`               |
| **Webhooks**              | Not exposed (use the `allfeat:complete` Custom Event)         | Yes — signed `work_registered` / `work_updated` / `work_failed` POSTs  |
| **Update flow**           | User enters access code, **or** picks from a ref-scoped work-selector | Your backend calls `/v1/works/{ats_id}/versions/...`, including `external_user_ref` in the body when the work was tagged at register time (otherwise omit) |
